...
public:
	network_socket_t(sock_type_t _sockfd);
	~network_socket_t();
	int handleWrite();
	size_t handleRead();
	void initAddress();
...
struct network_io::Impl {
    ...
    bool openUDPSocket(const char *host, int port, std::shared_ptr<network_conn_t>& out);
	void update();
...
network_socket_t::network_socket_t(sock_type_t _sockfd)
	: network_conn_t(), sockfd(_sockfd) {
...
bool network_io::openUDPSocket(const char *host, int port, std::shared_ptr<network_conn_t>& out) {
    return _M_impl->openUDPSocket(host, port, out);
}
...
void network_io::Impl::acceptPendingConnections(network_socket_t* netSocket) {
	while (1) {
		...
		std::shared_ptr<network_socket_t> remote = std::make_shared<network_socket_t>(sockfd);
...
bool network_io::Impl::listenAt(const char *host, int port, protocol_type_i32 protocol, std::shared_ptr<network_conn_t>& out) {
	...
	std::shared_ptr<network_socket_t> mainSocket = std::make_shared<network_socket_t>(rawSocket);
...
bool network_io::Impl::openUDPSocket(const char *host, int port, std::shared_ptr<network_conn_t>& out) {
    struct addrinfo hints, *ai = nullptr;
    std::unique_ptr<addrinfo, addrinfo_deleter> deleter(ai);
    int err;
    char buf[64];
