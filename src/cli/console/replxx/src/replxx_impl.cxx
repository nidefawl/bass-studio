...
Replxx::ReplxxImpl::ReplxxImpl( FILE*, FILE*, FILE*, std::atomic<bool>* pShouldStop )
	: _utf8Buffer()
	, _data()
	, _charWidths()
	, _display()
...
char32_t Replxx::ReplxxImpl::do_complete_line( bool showCompletions_ ) {
	...
	if ( _doubleTabCompletion ) {
		...
		do {
			c = read_char();
		} while ( !_pShouldStop->load() && c == static_cast<char32_t>( -1 ) );
