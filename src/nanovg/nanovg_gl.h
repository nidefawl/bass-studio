...
extern const char* NVG_GLSL_FRAG;
extern const char* NVG_GLSL_VERT;
#ifdef __cplusplus
}
#endif
#endif /* NANOVG_GL_H */
...
static int glnvg__renderCreate(void* uptr)
{
    ...
    const char* srcVertexShader = gl->overrideShaderVertex ? gl->overrideShaderVertex : NVG_GLSL_VERT;
    const char* srcFragmentShader = gl->overrideShaderFragment ? gl->overrideShaderFragment : NVG_GLSL_FRAG;
	...
	for (i = 0; i < npaths; i++) {
		...
		if (path->nfill > 0) {
			...
			for (int j = 0; j < path->nfill; j++) {
				gl->verts[offset+j].z = zPos;
				gl->verts[offset+j].w = 1.0f;
			}
#endif
			offset += path->nfill;
		}
...
static void glnvg__renderTriangles(void* uptr, NVGpaint* paint,
		NVGcompositeOperationState compositeOperation, NVGscissor* scissor,
								   const NVGvertex* verts, int nverts, float zPos, float fringe)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	GLNVGcall* call = glnvg__allocCall(gl);
	GLNVGfragUniforms* frag;
