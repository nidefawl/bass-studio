...
private:
	...
	void run2() {
        ...
        while (true){
            ...
            int32_t processedBlock;
            {
            	...
            	processedBlock = host->processPlaybackSamplePos(samplePos, tickPos, state, inLoop, isLoopAround);
            }
            /*
	...
	void run() {
        ...
        while (true){
            ...
            {
            	ThreadLock lock = this->lockThread();
            	processedBlock = host->processPlaybackBlockPos(blockPos, pos, state, inLoop);
            }
            /*
             * at sample rate 44100 and blocksize 512 the block duration is 1.xxms
             * the producer side trys to stay 4 blocks ahead of the consumer (audio thread)
