...
/* Set the number of parameters if the
...
/* Returns the number of microseconds per
...
/* Returns the number of seconds per quarter note.
   Returns -1.0 if the MIDI message is not a tempo meta message. */
...
/* Returns the tempo in terms of beats per minute. 
...
/* Returns the tempo in terms of ticks per seconds. */
...
/* Returns the tempo in terms of seconds per tick. */
...
/* Returns true if message is a Meta message (when the command byte is 0xff).*/
int MidiMessage::isMeta() const {
    if (size() == 0) {
        return 0;
    } else if ((*this)[0] != 0xff) {
        return 0;
...
int MidiMessage::isMetaMessage() const {
    return isMeta();
}
/* Returns true if the command nibble is 0x80 
   or if the command nibble is 0x90 with p2=0 velocity */
int MidiMessage::isNoteOff() const {
    if (size() != 3) {
        return 0;
...
/* Returns true if the command byte is in the 0x90 
   range and the velocity is non-zero */
int MidiMessage::isNoteOn() const {
    if (size() != 3) {
        return 0;
...
/* Returns true if either a note-on or a note-off message. */
int MidiMessage::isNote() const {
    return isNoteOn() || isNoteOff();
}

/* Returns true if the command byte is in the 0xA0 range. */
int MidiMessage::isAftertouch() const {
    if (size() != 3) {
        return 0;
...
/* Returns true if the command byte is in the 0xB0 range. */
int MidiMessage::isController() const {
    if (size() != 3) {
        return 0;
...
/* Returns true of a patch change message (command nibble 0xc0).*/
int MidiMessage::isTimbre() const {
    if (((*this)[0] & 0xf0) != 0xc0) {
        return 0;
    } else if (size() != 2) {
        return 0;
...
int MidiMessage::isPatchChange() const {
    return isTimbre();
}

/* Returns true of a channel pressure message (command nibble 0xd0). */
int MidiMessage::isPressure() const {
    if (((*this)[0] & 0xf0) != 0xd0) {
        return 0;
    } else if (size() != 2) {
        return 0;
...
/* Returns true of a pitch-bend message (command nibble 0xe0). */
int MidiMessage::isPitchbend() const {
    if (((*this)[0] & 0xf0) != 0xe0) {
        return 0;
    } else if (size() != 3) {
        return 0;
...
/* returns the meta-message type for the MidiMessage.
   If the message is not a meta message, then returns -1 */
...
/* Returns true if message is a meta message describing tempo (meta message type 0x51). */
int MidiMessage::isTempo() const {
    if (!isMetaMessage()) {
        return 0;
    } else if ((*this)[1] != 0x51) {
        return 0;
...
int MidiMessage::isEndOfTrack() const {
    return getMetaType() == 0x2f ? 1 : 0;
}

/* Return index 1 byte, or -1 if it doesn't exist. */
...
/* Return index 2 byte, or -1 if it doesn't exist. */
int MidiMessage::getP2() const {
    return size() < 3 ? -1 : (*this)[2];
}

/* Return index 3 byte, or -1 if it doesn't exist. */
int MidiMessage::getP3() const {
    return size() < 4 ? -1 : (*this)[3];
}

/* Return the key number (such as 60 for
...
/* Return the key veolocity.  If the message
...
/* Set the note on/off key number (or aftertouch key).
   Ignore if not note or aftertouch message.
...
/* Set the note on/off velocity.
  ignore if note a note message.
  Limits the input value to the range from 0 to 127. */
...
/* Returns the top 4 bits of the (*this)[0]
...
/* Return the command byte or -1 if not allocated. */
...
/* Returns the bottom 4 bites of the
...
/* Set the command byte and parameter bytes
...
/* Set the second and optionally the
...
/*  Set the contents of MIDI bytes to the input list of bytes. */
...
/* Encode a MidiPlus accidental state for a note. */
...
/* Return the diatonic pitch class and accidental for a note-on's key number.
   The MIDI file must be encoded with MIDIPlus pitch spelling codes for this
   function to return valid data; otherwise, it will return a neutral fixed
   spelling for each MIDI key.
...
/* create a time signature meta message (meta #0x58).
   The "bottom" parameter should be a power of two;
...
/* Create a Meta event with the given text std::string as the parameter.
   The length of the std::string should not be longer than 127 characters
   at the moment (will have to check if VLV sizes are allowed). */
...
/* Create a metaevent copyright message.
...
/* Create a metaevent track name message.
   This is not a real MIDI message, but rather a pretend message for use
   within Standard MIDI Files. */
...
/* Create a metaevent instrument name message.
   This is not a real MIDI message, but rather a pretend message for use
   within Standard MIDI Files. */
...
/* Create a metaevent lyrics/text message.
   This is not a real MIDI message, but rather a pretend message for use
   within Standard MIDI Files. */
...
/* Create a metaevent marker message.
   This is not a real MIDI message, but rather a pretend message for use
   within Standard MIDI Files. */
...
/* Create a metaevent cue-point message.
   This is not a real MIDI message, but rather a pretend message for use
   within Standard MIDI Files. */
