...
#define LOG_BUFFER_SIZE 4096
	...
	public:
		...
		void log(const char* data, size_t len) override {
			ALEPH_ASSERT_THROW(numCalls == 0);
			ALEPH_ASSERT_THROW(data != nullptr);
			ALEPH_ASSERT_THROW(len > 0);
			ALEPH_ASSERT_THROW(len < LOG_BUFFER_SIZE);
			ALEPH_ASSERT_THROW(data[len] == 0);
			ALEPH_ASSERT_THROW(safe_strlen(data, 1UL<<16U) == len);
