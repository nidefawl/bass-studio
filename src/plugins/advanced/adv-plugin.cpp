...
void GuiAdvPluginVST2::getParameterName (VstInt32 index, char* label)
{
	switch (index)
	{
	case kNoiseVolume:		vst_strncpy(label, "Noise level", kVstMaxParamStrLen);	return;
	}
}
...
void GuiAdvPluginVST2::setParameter (VstInt32 index, float value)
{
	...
	switch (index)
	{
	case kNoiseVolume:		ap->noiseVolume = value;  break;
	}
...
float GuiAdvPluginVST2::getParameter (VstInt32 index)
{
	...
	switch (index)
	{
	case kNoiseVolume:		value = ap->noiseVolume;	break;
	}
	return value;
}
...
//		dprintf("unsupported this->getAeffect()->numOutputs");
	}
}
