...
//		lW = 0;
	...
	int textHeight = TRACK_HEIGHT_STEP;//dsp_util::scaledRange(0, dsp_util::MTR_FLOOR, dsp_util::MTR_CEIL)*mtrSize.y;
//	int textMinHeight=12;
//	textHeight = math::max<int>(textMinHeight, textHeight);
//	if (mtrSize.y - textHeight < textMinHeight) {
//		textHeight = 0;
//	}
//	mtrSize.y-=textHeight;
//	textHeight = math:max(1)
	const double scaledZero = dsp_util::scaledRange(0, dsp_util::MTR_FLOOR, dsp_util::MTR_CEIL);
	float hZero = (1.0f - scaledZero) * mtrSize.y;
	float yZero = mtrPos.y + mtrSize.y - hZero;
...
//		lW = 0;
	...
	if (hasLegend && size.y > TRACK_HEIGHT_STEP*1.5) {
		...
		nvgStroke(vg);
		UTIL_setFont(vg, theme, lW*0.375, G_WHITE, NVG_ALIGN_BOTTOM | NVG_ALIGN_RIGHT);
		float prevStopY = mtrPos.y+textHeight;//dsp_util::scaledRange(0, dsp_util::MTR_FLOOR, dsp_util::MTR_CEIL)*mtrSize.y;
