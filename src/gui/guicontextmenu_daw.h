...
public:
    ...
    void initAdaptive() {
        fixed = false;
        entries.push_back({ GRID_WIDEST, 0, 0, 0, "Max" });
        entries.push_back({ GRID_WIDE, 0, 0, 0, "Large" });
        entries.push_back({ GRID_MED, 0, 0, 0, "Normal" });
        entries.push_back({ GRID_NARROW, 0, 0, 0, "Small" });
        entries.push_back({ GRID_NARROWEST, 0, 0, 0 + 15, "Min" });
    }
