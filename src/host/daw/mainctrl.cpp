...
#include "gui/track/trackctr_nodes.hpp"
#include "gui/track/trackctr.hpp"
#include "gui/track/trackmixers.hpp"
#include "gui/views/controls.hpp"
#include "gui/views/debugctr.hpp"
#include "gui/views/notify.hpp"
...
void DawInstance::resetDragDropClipboards() {
    dragdropclip.reset();
    dragdropclip = {};
}
...
public:
    DawViewContainersMain(DawCtrl* const _dawCtrl, ngui::MenuBar& menubar, DAW::Cursor& _cursor, DAW::TrackSelection& _trackSelection, project_t& _project, project_globals_t& _projectGlobals, dragdrop_file& dragdropclip, int32_t instanceId)
        : dawCtrl(_dawCtrl),
          ...
          ctrEntryMixers(createGuiCtrLayoutEntry(std::make_shared<guictr_mixers>(_dawCtrl, _cursor, _trackSelection, _project, _projectGlobals, dragdropclip))),
          ...
          ctr_clipeditorview()
    {
        ...
        ctrEntryMixers->setEntryTag(GuiContainerTag::TAG_MIXERS);
    ...
    void init() {
        ...
        ctrEntryMixers->removeEntryFromParent();
        ...
        if (dawCtrl->viewMode == view_mode_t::MIXER) {
            focusCtr = ctrEntryMixers.get();
        }
        if (dawCtrl->viewMode == view_mode_t::NODE_EDITOR) {
    ...
    void loadLayout(const dawview_layout_t& viewLayout) {
        ...
        context.entriesPreconstructed[GuiContainerTag::TAG_MIXERS] = { ctrEntryMixers };
        ...
        const std::array<TagGuiType, 5> tagGuiTypes = {
            TagGuiType{GuiContainerTag::TAG_TRACKS, gui_type::CTR_TYPE_TRACKS, &ctrEntryTracks},
            TagGuiType{GuiContainerTag::TAG_MIXERS, gui_type::CTR_TYPE_MIXERS, &ctrEntryMixers},
        ...
        } else if (ctrEntryMixers->isVisible()) {
            dawCtrl->viewMode = view_mode_t::MIXER;
...
void DawCtrl::removeTrackFromView(track_t* track, int flags) {
    ...
    view->visitEntries([track, flags, &nTrackViews](SPLayoutEntry& entry) {
        ...
        if (entry->getType() == gui_type::CTR_TYPE_MIXERS) {
            guictr_cast<guictr_mixers>(entry)->removeTrack(track, flags);
        }
        if (entry->getType() == gui_type::CTR_TYPE_NODES) {
            guictr_cast<guictr_nodes_splitview>(entry)->refresh();
        }
...
bool DawCtrl::handleGlobalCommand(DAW::UI::CommandContext& ctxt) {
    ...
    view->visitEntries([&ctxt, &bHandled](SPLayoutEntry& entry) {
        ...
        if (entry->getType() == gui_type::CTR_TYPE_MIXERS
            && guictr_cast<guictr_mixers>(entry)->handleEditorCommand(ctxt)) {  
            bHandled = true;
            return false;
        }
        if (entry->getType() == gui_type::CTR_TYPE_CLIPEDITOR
...
void DawCtrl::prerender(NVGcontext* nanovgCtxt, int32_t x, int32_t y, int32_t w, int32_t h, float pixelRatio) {
    ...
    hires_timer_t timer;
    BaseCtrl::prerender(nanovgCtxt, x, y, w, h, pixelRatio);
...
void DawInstance::updateLoadAudioTasks() {
    auto it = this->loadAudioTasks.begin();
    while (it != this->loadAudioTasks.end()) {
        std::shared_ptr<DAW::LoadAudioTask>& task = *it;
        if (task->isCompleted()) {
            task->finishMainThread(this);
            it = this->loadAudioTasks.erase(it);
        } else {
            ++it;
        }
    }
...
bool DawInstance::preloadDraggedFiles(const std::vector<String>& files) {
    ...
    for (auto path : files) {
        ...
        if (ext.empty() && PathIsDirectory(path)) {
            dragdropclip.type = Type::TYPE_DIRECTORY;
        ...
        if (stl_contains(std::array{ SUPPORTED_AUDIO_FILE_TYPES }, ext)) {
            dragdropclip.type = Type::TYPE_AUDIOFILE;
            auto audiocache = getAudioCache();
            audiocache::fileloader loader;
            loader.setTargetSampleRate(audiocache->getSampleRate());
            if (!loader.resolveFile(path, lastProjectDirectory, false)) {
            ...
            if (!loader.isOk()) {
                ...
                if (sample) {
                    ...
                    auto host = getHost();
                    dbgassert(host);
                    clip.setLen(sampleToTickConvert<tick_t, roundmode::round>(lenSamples, projectGlobals.tempo100, host->m_sampleFormatInternal.sampleRate));
                ...
                // move loader to async task
                auto task = std::make_shared<DAW::LoadAudioTask>(path, std::move(loader));
                if (!workerThread.pushTask(task.get())) {
                    dragdropclip.state = State::STATE_FAILED_LOADING;
                    return false;
                }
...
void GrooveLibrary::loadGrooves() {
