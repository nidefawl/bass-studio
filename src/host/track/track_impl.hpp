...
#include "host/track/audiotrack.hpp"
...
struct audio_stage_t : public IDelayLineStorage {
    ...
    virtual void getNotesDelayed(tick_t blockStart, tick_t blockEnd, const double ticksPerBlock, std::vector<midievent_note_t>& evtsOut, std::vector<DAW::Host::midievent_ctrl_t>& ctrlEvts, bool isPost, int32_t midiChannelMatch, int32_t midiChannelRewrite);
