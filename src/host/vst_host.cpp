...
void vsthost::updateTime(VstTimeInfo& timeinfo, int32_t samplePos, double dTickPos, playback_state state) const {
	...
	int32_t samplePosTick = PPQ24TickSample(midiTick, prjGlobals.tempo100, sampleFormat.sampleRate, sampleFormat.blockSize);
...
//						log_printf("%d note KILL %s %d\n", noteName(noteHeld.pitch), noteHeld.start());
	...
	if (notesProcessed) {
		...
		if (state == playback_state::status_play && prjGlobals.recordArmed) {
...
//		log_printf("Realtime midi notes %d\n", midiRealtimeInput->m_list.size());
	...
	if (recordingClip && !(state == playback_state::status_play && prjGlobals.recordArmed)) {
		for (auto& note : midiRealtimeInput->m_list) {
...
void vsthost::processAudio(audio_stage_t* stage, AudioBlock* input, AudioBlock* output, int32_t samplePos, int32_t numSamples, playback_state state) const {
	...
	lastProcessingGraphs[stage->stageId] = processingGraph;
	AudioBlock tempBlock(8, 256);
	hires_timer_t timer;
	int64_t timeTotal = 0;
	for (auto itAudioStage = processingGraph->nodesFlatOrdered.begin(); itAudioStage != processingGraph->nodesFlatOrdered.end(); itAudioStage++) {
		...
		switch (effNode.type) {
			...
			case DAW::track_node_type_t::AUDIOSTAGE:
				dbgassert(effNode.stageId == stage->stageId);
...
//		tmp.timer.reset();
	...
	//					}
				...
				if (DAW::resolveAudioChannel(this, numChannelsTrack, tracksrc.channel, /*ptrExternalInputs*/ nullptr, src)) {
					...
					dbgassert(delayToMaxInputLatency >= 0);
					DelayLine* delayLine = stage->getEffectDelayLine(tracksrc.trackEdgeId, numChannels);
//					AudioBlock& tempBlock = tmp.tempBlock;
