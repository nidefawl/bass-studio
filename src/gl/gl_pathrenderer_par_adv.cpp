...
namespace GLPathRendererParAdvSrc {
...
int GLPathRendererParAdvanced::init() {
    GLuint vertex_shader = compileShader(GL_VERTEX_SHADER, GLPathRendererParAdvSrc::srcShaderVertex);
    if (!vertex_shader) {
        return 1;
    }
    GLuint fragment_shader = compileShader(GL_FRAGMENT_SHADER, GLPathRendererParAdvSrc::srcShaderFragment);
    if (!fragment_shader) {
        return 1;
    }
...
void GLPathRendererParAdvanced::bakePaths(std::vector<vec2list> paths, Uniforms pathOpt, BakeGLPath& out) {
...
void GLPathRendererParAdvanced::render(BakeGLPath& bakedPath, const mat4x4& matProj, const mat4x4& matView, const mat4x4& matModel) {
